...
Vaults have a `version` field (returned by `/vault-data`). Each version introduced new features. When the user asks about vault versions or capabilities, reference the relevant changelog:

- **v2** — first major upgrade: improved fee structure, better reward distribution. Details: <https://blog.stakewise.io/productUpdate/introducing-vaults-v2-0-optional-upgrade-now-available>
- **v3** — added osETH minting, boost module, and ERC-20 vault tokens. Details: <https://blog.stakewise.io/productUpdate/vaults-v3-0-optional-upgrade-now-available>
- **v4/v5** — latest upgrade: enhanced performance, new configuration options. Details: <https://blog.stakewise.io/productUpdate/vaults-v4-0-optional-upgrade-now-available>

Upgrades are optional — vault operators choose when to upgrade, so vaults may run different versions. Fetch the linked page for full details when the user asks.
...
| GET | `/staked-vaults` | — | List **every** vault where the saved address has any position (stake, exit request, or leverage) — returns vault names, addresses, APY, TVL, and per-user totals |
| GET | `/vault-data` | `vaultAddress` | Public vault info — name, description, APY, base APY, fee, capacity, utilization, osETH minting config (LTV), ERC20 token info. Does **not** require a saved user address |
| GET | `/vault-stats` | `vaultAddress`, `days` (optional, default 30, max 365) | Historical vault performance by day — APY, TVL (balance), and rewards for each day, plus a summary with average APY and total rewards. Does **not** require a saved user address |
| GET | `/user-stats` | `vaultAddress`, `days` (optional, default 30, max 365) | Historical **user** performance in a specific vault — personal APY, balance, and rewards (with breakdown into stake/boost/extra rewards) by day. **Requires** a saved user address |
| GET | `/vault-balance` | `vaultAddress` | Detailed user position (stake, minted osETH, boosted osETH, rewards, user APY) for one specific vault |
| GET | `/vault-queue` | `vaultAddress` | Status of the unstake **and** unboost queues for one specific vault |
| GET | `/created-vaults` | — | List vault addresses created (administered) by the saved user address. Use this to discover vaults the user owns, then call `/vault-data` or `/vault-stats` for details |
| GET | `/vault-whitelist` | `vaultAddress` | Whitelist of addresses allowed to stake in a private vault. If the user's address is saved, also reports whether it is whitelisted. Does **not** require a saved user address, but benefits from one |
...
Some endpoints (`/vault-data`, `/vault-stats`, `/vaults-list`) work without a saved address — they return public vault data. All other data endpoints require the user's address to be saved first (via `/save-address`). The server keeps the address in memory.
...
Call `/staked-vaults` — it gives the full overview across every vault where the user has a position.

```bash
curl -sS "http://127.0.0.1:5165/staked-vaults"
```
...
1. If the user gave a **vault address**, call `/vault-balance?vaultAddress=0x...` directly.
2. If the user gave a **vault name** (e.g. "Genesis Vault"), you do not know its address yet. First call `/staked-vaults`, find the vault whose `displayName` matches (case-insensitive, fuzzy match is fine), then call `/vault-balance` with its address.
...
Call `/vault-data?vaultAddress=0x...` — returns public vault information (APY, fee, capacity, osETH config, etc.). This endpoint does **not** require a saved user address, so you can call it without `/save-address`.

If the user refers to a vault by name, resolve the address via `/staked-vaults` first (if the user has deposits there), or ask the user for the vault address directly.

```bash
curl -sS "http://127.0.0.1:5165/vault-data?vaultAddress=0x..."
```

### "How has this vault performed?" / "Show vault stats for the last week"

Call `/vault-stats?vaultAddress=0x...` — returns daily APY, TVL, and rewards over a time period. By default returns the last 30 days. Use the `days` query param to customize (e.g. `days=7` for a week, `days=90` for a quarter, max 365).
...
```bash
curl -sS "http://127.0.0.1:5165/vault-stats?vaultAddress=0x...&days=7"
```
...
Call `/user-stats?vaultAddress=0x...` — returns the **user's personal** daily APY, balance, and rewards in a specific vault. Unlike `/vault-stats` (which shows vault-wide data), this shows only the user's position history. Rewards include a breakdown into stake rewards, boost rewards, and extra rewards when available.

```bash
curl -sS "http://127.0.0.1:5165/user-stats?vaultAddress=0x...&days=30"
```

Use `/vault-stats` when the user asks about a vault's overall performance, and `/user-stats` when they ask about their own rewards/earnings history. The same `days` param applies (default 30, max 365).

### "When will my withdrawal be ready?" / "Check my exit queue"

Call `/vault-queue?vaultAddress=0x...`. Resolve the vault address the same way as for balance queries (via `/staked-vaults` if only a name was given).
...
Call `/created-vaults` — returns addresses of vaults the user has created (i.e. where they are the admin). This is useful when the user is a vault operator, not just a depositor.

```bash
curl -sS "http://127.0.0.1:5165/created-vaults"
```

Once you have the addresses, you can call `/vault-data`, `/vault-stats`, or any other vault endpoint to get detailed information about each created vault.

### "Can I stake in this vault?" / "Am I whitelisted?" / "Who can stake in this vault?"

Call `/vault-whitelist?vaultAddress=0x...` — returns the list of addresses allowed to stake in a private vault. If the user's address is saved, the response also indicates whether they are whitelisted.

```bash
curl -sS "http://127.0.0.1:5165/vault-whitelist?vaultAddress=0x..."
```

If the vault is not private or has no whitelist entries, tell the user. If they ask specifically whether they can stake, check `isUserWhitelisted` in the response data.
...
- **Focus on what matters.** Show the data that is relevant to the user's question. Omit zero or empty values (e.g. if the user has no minted osETH or no boosted position in a vault, don't show "Minted: 0 osETH" or "Boosted: 0 osETH"). When the user asks a specific question (e.g. "tell me about this vault"), answer that question — don't automatically pull in extra data like 30-day stats unless the user asks for it. Use your judgment to keep the response concise and useful.
- Each response contains two representations: `result` (pre-formatted markdown with human-readable numbers) and `data` (raw JS object). **Prefer values from `result`** when presenting to the user — they are already formatted with proper units, separators, and structure. Use `data` only when you need to do arithmetic (totals, comparisons) or the user explicitly asks for raw/exact figures.
...
- **`error: "The vault address provided is invalid."`** → the `vaultAddress` query param was malformed. Re-resolve via `/staked-vaults`.
- **Empty result** (`ok: true` with empty `data`) from `/staked-vaults`, `/created-vaults`, or `/vaults-list` → the address simply has nothing on StakeWise. Tell the user clearly.
...
- Do not call user-specific endpoints (`/staked-vaults`, `/vault-balance`, `/vault-queue`, `/user-stats`, `/created-vaults`) before you have a valid Ethereum address saved.
- Do not guess, fabricate, or "round" balances, APYs, or queue times.
- Do not assume `/staked-vaults` covers vaults with zero balance — it only returns vaults where the address actively has a position.
- Do not call `/vault-balance` or `/vault-queue` with a vault name string — these endpoints need a hex address.
